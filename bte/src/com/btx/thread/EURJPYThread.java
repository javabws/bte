...
public class EURJPYThread implements Runnable {
	...
	public void threadStart(EURJPYThread c) {
		t = new Thread(c);
	...
	public double getExpiryValue() {
		...
		String url = "http://binarytradexchange.com:8080/feed/getEURJPY.jsp";
		double yvalue=0.0;
        try {
	...
	// ------Grouping Method------//
		...
		private void doGroup() throws SQLException, ParseException {
			...
			st = conn
					.prepareStatement("select userid,amount,user,type,hour,minute,second,value from eurjpy where year=? and month=? and date=? and hour=? and (minute=? and second>56) and status=?");
			...
			st = conn
					.prepareStatement("select userid,amount,user,type,hour,minute,second,value from eurjpy where year=? and month=? and date=? and hour=? and minute=? and status=?");
			st.setInt(1, utc.getYear());
			st.setInt(2, utc.getCurrentTimeStamp().getMonth());
			st.setInt(3, utc.getDate());
			st.setInt(4, utc.getHour());
...
//			itr2 = fput.iterator();
		...
		synchronized public  void updateLost(long userid, double lost) throws SQLException {
			st = conn
					.prepareStatement("update eurjpy set lost=?,gtype='lost' where userid=?");
			st.setDouble(1, lost);
			st.setLong(2, userid);
			st.executeUpdate();
		}
		...
		synchronized public  void updateEarned(long userid, double earned) throws SQLException {
			st = conn
					.prepareStatement("update eurjpy set earned=?,gtype='won' where userid=?");
		...
		{
			st = conn
					.prepareStatement("update eurjpy set expiryvalue=? where userid=?");
