...
import com.btx.logic.CalculateBean;
import com.btx.logic.CalculateMethods;
import com.btx.logic.DbDriver;
import com.btx.logic.UTCTime;
...
public class PlayerHandler implements Runnable {
	...
	// ------Grouping Method------//
		...
		private void doGroup(String at) throws SQLException, ParseException {
			...
			st = conn
					.prepareStatement("select userid,amount,user,type,hour,minute,second,value from "+at.toLowerCase()+" where year=? and month=? and date=? and hour=? and (minute=? and second>56) and status=?");
			...
			st = conn
					.prepareStatement("select userid,amount,user,type,hour,minute,second,value from "+at.toLowerCase()+" where year=? and month=? and date=? and hour=? and minute=? and status=?");
			st.setInt(1, utc.getYear());
			st.setInt(2, utc.getCurrentTimeStamp().getMonth());
			st.setInt(3, utc.getDate());
			st.setInt(4, utc.getHour());
...
//-----------Calculating Winner----//
		...
		private void calculateWinner(String at) throws SQLException {
			
			System.out.println("Calculate method invoked");
//			gcall = new ArrayList<CalculateBean>();
//			gput = new ArrayList<CalculateBean>();
//			
			expiryValue =getExpiryValue(at);
//			itr1 = fcall.iterator();
//			itr2 = fput.iterator();
			...
			while (itr3.hasNext()) {
				...
				{
					win="call";
					totalp=winIteration(totalp, fcItr,win,at);//--getting the amount from put users and transfered to call users
				...
				{
					win="put";
					totalc=winIteration(totalc, fpItr,win,at);//--getting the amount from call users and transfered to put users
		...
		String refer="";
		private double winIteration(Double to,@SuppressWarnings("rawtypes") Iterator itr,String type,String at) throws SQLException
		{
			...
			while (itr.hasNext() && total!=0.0) 
			{
				...
				if(total==ca.getAmount())
				{
					...
					updateEarned(ca.getUserid(),earned,at);
				...
				else if(total<ca.getAmount())
				{
					...
					updateEarned(ca1.getUserid(),earned,at);
					//updateLost(ca.getUserid(),lost);
					CalculateMethods.updateWallet(wtype, earned, ca.getUser());
					CalculateMethods.updateCompanyWallet(type, company, ca.getUser());
					System.out.println("Total<Amount");
				...
				updateExpiryValue(ca.getUserid(),expiryValue,at);
		...
		private double lostIteration(Double to,@SuppressWarnings("rawtypes") Iterator itr,String at) throws SQLException
		{
			...
			lositr=los.iterator();
				...
				while(lositr.hasNext())
				{
					...
					System.out.println("Balance: "+total);
						...
						updateLost(ca.getUserid(), ca.getAmount(),at);
		...
		synchronized public  void updateLost(long userid, double lost,String at) throws SQLException {
			st = conn
					.prepareStatement("update "+at.toLowerCase()+" set lost=?,gtype='lost' where userid=?");
			st.setDouble(1, lost);
			st.setLong(2, userid);
			st.executeUpdate();
		}
		...
		synchronized public  void updateEarned(long userid, double earned,String at) throws SQLException {
			st = conn
					.prepareStatement("update "+at.toLowerCase()+" set earned=?,gtype='won' where userid=?");
			st.setDouble(1, earned);
			st.setLong(2, userid);
			st.executeUpdate();
		}

		synchronized public void updateExpiryValue(long userid,double expiry,String at) throws SQLException
		{
			st = conn
